...
(function(global) {
  ...
  var GATrainer = function(net, options_, initGene) {
    ...
    this.num_opponents = typeof options.num_opponents !== 'undefined' ? options.num_opponents : 2;
  ...
  GATrainer.prototype = {
    ...
    matchTrain: function(matchFunc) { // uses arms race to determine best chromosome by playing them against each other
      ...
      var opponent;
      var fitness;
      var numOpponents = this.num_opponents;
      var c = this.chromosomes;
      var result = -1;
      N = this.population_size;
      ...
      for (i = 0; i < N; i++) {
          var chosenChromosomes = [];
          chosenChromosomes.push(c[i]);
          while (chosenChromosomes.length < numOpponents) {
              let opponent = randi(0, N);
              if(!chosenChromosomes.includes(c[opponent])) {
                chosenChromosomes.push(c[opponent]);
              }
          }

          result = matchFunc(chosenChromosomes);
          for(j = 0; j < numOpponents; j++) {
            c[j].nTrial += 1;
            if(result > -1) {
              c[j].fitness += 2*(c[j] === result); //2 for the winner, 0 for all others
            }
            else {
              c[j].fitness = 1;
            }
